...
class FirebaseLogger:
    def __init__(self, key_path):
        ...
        self.session_id = None  # <-- RESTORED
    
    def get_active_session_id(self): # <-- RESTORED
        try:
    ...
    def sync_conversation_start(self, output_data):
        ...
        try:
            ...
            # --- DELAY ADDED ---
            # Give React 1.5 seconds to archive the final line before finishing
            print("⏳ Waiting for React to finalize the last archive...")
            time.sleep(1.5)
            
            self.db.collection("emotion").document("current").update({"transcribed": True})
            print("✅ 'Done' signal sent to website!")
                
        except Exception as e:
            print(f"❌ Firebase Transcription Sync Failed: {e}")
    
    def archive_emotion_change(self, emotion, accuracy, emotion_timestamp):
        # RESTORED: Ensure we have the Session ID before archiving
        ...
        archive_data = {
            ...
            "sessionID": self.session_id, # <-- RESTORED
            "archived": True
        }
        try:
